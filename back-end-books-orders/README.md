...
Se registra en **Eureka** como `books-orders` y utiliza **WebClient** con `@LoadBalanced` para resolver las URLs de
otros microservicios vía Service Discovery. Escucha en el **puerto 8080**.
...
- `getRecentOrders()`: Obtiene las ordenes** más recientes** del usuario (ordenadas por fecha descendente). Para cada
  ítem de la orden, consulta al catálogo para obtener el nombre y precio actual del libro.
